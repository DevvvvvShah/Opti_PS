...
from getting_from_greedy import get_from_greedy
...
#         print(f"Time: {time:.2f} seconds, Objective: {obj:.2f}")
...
def container_loading_with_relative_constraints():
   ...
   #  priority_penalty *= 5000
    ...
    for carton in cartons:
        if carton['id'] != 'P-103':
            continue
        # print(f"xi = {xi[carton['id']].Start}, yi = {yi[carton['id']].Start}, zi = {zi[carton['id']].Start}")
        # print(f"lx = {orientation[carton['id']]['lx'].Start}, ly = {orientation[carton['id']]['ly'].Start}, lz = {orientation[carton['id']]['lz'].Start}")
        # print(f"wx = {orientation[carton['id']]['wx'].Start}, wy = {orientation[carton['id']]['wy'].Start}, wz = {orientation[carton['id']]['wz'].Start}")
        # print(f"hx = {orientation[carton['id']]['hx'].Start}, hy = {orientation[carton['id']]['hy'].Start}, hz = {orientation[carton['id']]['hz'].Start}")
        print(f"len = {carton['length']}, wid = {carton['width']}, hei = {carton['height']}")
    penalty = 5000 * sum(
        max(sij[(carton['id'], container['id'])] * carton['priority'] for carton in cartons) for container in
        containers) + sum((1-(sum(sij[(carton['id'], container['id'])] for container in containers))) * carton['cost'] for carton in cartons)
...
def container_model_with_correct_objectives():
   ...
   # Objective:
    penalty = 5000 * sum(pj[container['id']] for container in containers) + sum((1-(sum(sij[(carton['id'], container['id'])] for container in containers))*carton['cost']) for carton in cartons)
    model.setObjective(penalty, GRB.MINIMIZE)
    model.optimize()
    # Extract the solution
    if model.status == GRB.OPTIMAL:
...
def container_loading_with_relative_constraints_2():
    """
    Solve the 3D container loading problem using mixed integer programming,
    incorporating relative positioning constraints (aik, bik, cik, dik, eik, fik).
    ...
    # print(cartons)
    # Objective: Minimize unused space
    model.setObjective(gp.quicksum((gp.quicksum(sij[(carton['id'], container['id'])] for carton in cartons) for container in containers)), GRB.MAXIMIZE)
   # Objective: Minimize unused space
   #  priority_penalty = 0
   #  for container in containers:
   #     prod = 0
